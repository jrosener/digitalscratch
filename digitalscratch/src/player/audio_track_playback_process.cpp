...
Audio_track_playback_process::Audio_track_playback_process(QSharedPointer<Audio_track>         &in_at,
...
bool
Audio_track_playback_process::reset_sampler(unsigned short int in_sampler_index)
{
    this->sampler_current_samples[in_sampler_index] = 0;
    this->sampler_remaining_times[in_sampler_index] = 0;

    return true;
}

void
Audio_track_playback_process::del_sampler(unsigned short int in_sampler_index)
{
    this->reset_sampler(in_sampler_index);
    this->set_sampler_state(in_sampler_index, false);
    emit sampler_remaining_time_changed(0, in_sampler_index);
    this->at_samplers[in_sampler_index]->reset();

    return;
}

bool
Audio_track_playback_process::play_empty(unsigned short int   in_nb_samples,
                                         float              **out_samples)
{
...
bool
Audio_track_playback_process::play_audio_track(unsigned short int   in_nb_samples,
                                               float              **out_samples)
{
    // Prevent sample table overflow if going forward.
    if ((this->param->get_speed() >= 0.0) &&
       ((this->current_sample + 1) > (this->at->get_end_of_samples() - in_nb_samples)))
    {
        qCWarning(DS_PLAYBACK) << "audio track sample table overflow";
        this->play_empty(in_nb_samples, out_samples);
...
bool
Audio_track_playback_process::get_sampler_state(unsigned short int in_sampler_index)
{
    return this->sampler_current_states[in_sampler_index];
}

bool
Audio_track_playback_process::set_sampler_state(unsigned short int in_sampler_index,
                                                bool               in_state)
{
    this->sampler_current_states[in_sampler_index] = in_state;
...
bool
Audio_track_playback_process::is_sampler_loaded(unsigned short int in_sampler_index)
{
    bool result = false;

    if (this->at_samplers[in_sampler_index]->get_end_of_samples() == 0)
    {
        result = false;
    }
    else
    ...
    for (int i = 0; i < this->nb_samplers; i++)
    {
        ...
        if (this->at_samplers[i]->get_end_of_samples() > 0)
        {
            ...
            if (this->get_sampler_state(i) == true)
            {
                ...
                if ((this->sampler_current_samples[i] + 1) <= (this->at_samplers[i]->get_end_of_samples() - (in_nb_samples * 2)))
                {
                    ...
                    for (int j = 0; j < in_nb_samples; j++)
                    {
                        ...
                        out_samples[index][j]   = out_samples[0][j] + sample - (out_samples[index][j] * sample);
                        sample                  = (float)*(sample_pointer++) / (float)SHRT_MAX;
                        out_samples[index+1][j] = out_samples[1][j] + sample - (out_samples[index+1][j] * sample);
                        sample_pointer++;
                    }
                    this->sampler_current_samples[i] += in_nb_samples * 2;
                }
                else
                {
...
bool
Audio_track_playback_process::update_sampler_remaining_time(unsigned short int in_sampler_index)
{
    // Calculate remaining time in msec.
    this->sampler_remaining_times[in_sampler_index] = (unsigned int)(1000.0 * ((float)(this->at_samplers[in_sampler_index]->get_end_of_samples()
                                                                                            - this->sampler_current_samples[in_sampler_index]) + 1.0)
                                                                                            / (2.0 * (float)this->at_samplers[in_sampler_index]->get_sample_rate()));

    if (this->sampler_remaining_times[in_sampler_index] > 0)
        this->sampler_remaining_times[in_sampler_index] -= 1;

    // Send remaining time.
    emit sampler_remaining_time_changed(this->sampler_remaining_times[in_sampler_index], in_sampler_index);
...
Audio_track_playback_process::run(unsigned short int  in_nb_samples,
                                  ...
                                  float              *out_samples_2)
{
    float *samples[2] = { out_samples_1, out_samples_2 };
    ...
    {
        this->play_empty(in_nb_samples, samples);
    }
    else
    {
        // If track is loaded, play it.
        if (this->play_audio_track(in_nb_samples, samples) == false)
        {
...
bool
Audio_track_playback_process::play_data_with_playback_parameters(unsigned short int   in_nb_samples,
                                                                 float              **out_samples)
{
    ...
    this->src_data->output_frames     = in_nb_samples;
...
bool
Audio_track_playback_process::change_volume(float              *in_buffer,
...
bool
Audio_track_playback_process::jump_to_position(float in_position)
{
    // Calculate position to jump (0.0 < in_position < 1.0).
    unsigned int new_pos = (unsigned int)((float)in_position * (float)this->at->get_max_nb_samples());
    if (new_pos % 2 != 0)
    {
        new_pos++;
    }
...
float
Audio_track_playback_process::get_cue_point(unsigned short int in_cue_point_number)
{
    return this->sample_index_to_float(this->cue_points[in_cue_point_number]);
}

bool
Audio_track_playback_process::read_cue_point(unsigned short int in_cue_point_number)
{
    ...
    if (this->at->get_hash() != "")
    {
        data_persist->get_cue_point(this->at, in_cue_point_number, position);
        this->cue_points[in_cue_point_number] = this->msec_to_sample_index(position);
    }
...
bool
Audio_track_playback_process::store_cue_point(unsigned short int in_cue_point_number)
{
    // Store cue point.
    this->cue_points[in_cue_point_number] = this->current_sample;
    ...
    return data_persist->store_cue_point(this->at,
                                         in_cue_point_number,
                                         this->sample_index_to_msec(this->cue_points[in_cue_point_number]));
}

bool
Audio_track_playback_process::jump_to_cue_point(unsigned short int in_cue_point_number)
{
    // Jump
    this->current_sample = this->cue_points[in_cue_point_number];

    return true;
}

bool
Audio_track_playback_process::delete_cue_point(unsigned short int in_cue_point_number)
{
    // Delete cue point from playback process list.
    this->cue_points[in_cue_point_number] = 0;

    // Delete cue point from database.
    Data_persistence *data_persist = &Singleton<Data_persistence>::get_instance();
    return data_persist->delete_cue_point(this->at, in_cue_point_number);
}
...
QString
Audio_track_playback_process::get_cue_point_str(unsigned short in_cue_point_number)
{
    return Utils::get_str_time_from_sample_index(this->cue_points[in_cue_point_number],
                                                 this->at->get_sample_rate(),
                                                 true);
}

float
Audio_track_playback_process::sample_index_to_float(unsigned int in_sample_index)
{
    // Convert a sample index to a float position (from 0.0 to 1.0).
    return (float)((float)in_sample_index / (float)this->at->get_max_nb_samples());
}

unsigned int
Audio_track_playback_process::sample_index_to_msec(unsigned int in_sample_index)
{
    // Convert a sample index to milliseconds.
    return qRound((1000.0 * (float)in_sample_index) / (2.0 * (float)this->at->get_sample_rate()));
}

unsigned int
Audio_track_playback_process::msec_to_sample_index(unsigned int in_position_msec)
{
    // Convert a position from msec to sample index.
    return qRound(((float)in_position_msec * 2.0 * (float)this->at->get_sample_rate()) / 1000.0);
}
